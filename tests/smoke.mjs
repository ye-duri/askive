...
context.enterPermissionForTest();
await el('permission-camera-refresh').onclick();
assert.equal(el('permission-camera').children[1].textContent,'USB Camera');
el('permission-camera').value='usb-camera';await el('permission-camera').onchange();
await el('permission-start').onclick();assert.equal(el('camera-test').hidden,false);await el('permission-next').onclick();
...
el('timer').value='3';const interrupted=el('capture').onclick();assert.equal(el('permission-camera').disabled,true);
const before=deviceRequests.length;el('permission-camera').value='';await el('permission-camera').onchange();assert.equal(deviceRequests.length,before);
...
assert.equal(context.document.body.classList.contains('is-shooting'),true);
assert.equal(el('permission-camera').disabled,true);
await el('start').onclick();assert.equal(deviceRequests.at(-1).video.deviceId.exact,'usb-camera');
assert.equal(el('capture').disabled,false);assert.equal(context.readSession().photos,kept);
const oldTrack=externalTracks.at(-1);await el('start').onclick();assert.equal(oldTrack.stopped,true);
