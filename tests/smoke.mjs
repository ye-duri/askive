...
el('timer').value='3';const interrupted=el('capture').onclick();assert.equal(el('setup-camera').disabled,true);
const before=deviceRequests.length;el('setup-camera').value='';await el('setup-camera').onchange();assert.equal(deviceRequests.length,before);
...
assert.equal(el('setup-camera').disabled,true);
...
console.log('PASS: setup-only camera selection, exact external device, disconnect cancellation, preserved photos and same-camera reconnect. Mock devices only.');
