...
import {localPhotos} from './lib/local-photos.mjs';
...
export function createApp(env=process.env){
 const galleryEnv={CLEANUP_ENABLED:env.GALLERY_DIR?'1':'0',KIOSK_SECRET:env.KIOSK_SECRET,PHOTOS:env.GALLERY_DIR?localPhotos(path.resolve(env.GALLERY_DIR)):null};
 let publicOrigin='';
 ...
 const server=http.createServer({maxHeaderSize:8192,requestTimeout:15000,headersTimeout:10000},async(req,res)=>{
  ...
  try{
   ...
   if(url.pathname.startsWith('/api/gallery/')){
    const request=new Request((publicOrigin||`http://${req.headers.host}`)+req.url,{method:req.method,headers:req.headers,...(!['GET','HEAD'].includes(req.method)?{body:Readable.toWeb(req),duplex:'half'}:{})});
    const response=await gallery(request,galleryEnv);res.writeHead(response.status,Object.fromEntries(response.headers));res.end(Buffer.from(await response.arrayBuffer()));return;
   }
   ...
   if(!['/index.html','/styles.css','/print.css','/app.js','/media-ready.mjs','/delivery.js','/frames.json','/gallery.html','/gallery.js','/gallery.css','/vendor/qrcode.mjs','/fonts/nanum-pen.ttf','/fonts/OFL.txt','/fonts/SUIT-Variable.woff2','/fonts/MaruBuri-Regular.woff2','/fonts/SUIT-LICENSE.txt','/fonts/MaruBuri-LICENSE.txt'].includes(pathname)&&!/^\/frames\/[a-zA-Z0-9_-]+\.(svg|png|webp|jpe?g)$/.test(pathname))throw fail(404,'파일이 없습니다.');
 ...
 if(galleryEnv.PHOTOS){const timer=setInterval(()=>cleanup(galleryEnv.PHOTOS).catch(()=>{}),600000);timer.unref();server.on('close',()=>clearInterval(timer));void cleanup(galleryEnv.PHOTOS).catch(()=>{});}
 return server;
}
